...
2. 「Git更新」「ログ Git」等の指示はcommit＋push（サブモジュール含む）まで一気に完了させる
3. 日本語文字列はechoパイプ経由
   ```bash
...
```bash
curl -X POST http://localhost:<ポート>/mcp/ -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"ツール名","arguments":{}},"id":1}'
```
...
## MCPツール作成・更新時の必須手順
原則提案制。既存確認→ユーザー許可→作成→コンパイル確認待ち→RefreshAssets→登録確認
